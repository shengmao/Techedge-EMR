...
//#define databasePath = [[NSString alloc] initWithString:[docsDir stringByAppendingPathComponent:@"medicaldb02.sqlite"]];
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"MedicalDatamodel" withExtension:@"momd"];
    NSLog(@"%@",modelURL);
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    //NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"medicaldatamodel.sqlite"];
    NSString* documentsPath1 = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString *storePath1 = [documentsPath1 stringByAppendingPathComponent: @"medicaldb02.sqlite"];
    
    // Put down default db if it doesn't already exist
    NSFileManager *fileManager1 = [NSFileManager defaultManager];
    if (![fileManager1 fileExistsAtPath:storePath1]) {
        NSString *defaultStorePath1 = [[NSBundle mainBundle] pathForResource:@"medicaldb02" ofType:@"sqlite"];
        if (defaultStorePath1) {
            [fileManager1 copyItemAtPath:defaultStorePath1 toPath:storePath1 error:NULL];
        }
    }
    
    //NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"medicaldatamodel.sqlite"];
    NSString* documentsPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString *storePath = [documentsPath stringByAppendingPathComponent: @"medicaldatamodel.sqlite"];
    ...
    if (![fileManager fileExistsAtPath:storePath]) {
        NSString *defaultStorePath = [[NSBundle mainBundle] pathForResource:@"medicaldatamodel" ofType:@"sqlite"];
        if (defaultStorePath) {
