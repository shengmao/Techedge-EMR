...
- (void)viewDidLoad
{
    ...
    if (sqlite3_open(dbPath, &medicaldb)==SQLITE_OK) {
        ...
        NSString *querySQL = [NSString stringWithFormat:@"SELECT surname, name, stationroom, patientpic FROM tab_patient"];
        ...
        if (sqlite3_prepare_v2(medicaldb, query_stmt, -1, &statement, NULL)==SQLITE_OK) {
            ...
            while (sqlite3_step(statement)==SQLITE_ROW) {
                ...
                NSLog(@"database connection status 2.0");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"aaa%@",[array objectAtIndex:1]);
